...
def generate_html():
    ...
    <body>
    <div class="container">
        <button id="toggleDetailsBtn" class="toggle-btn" onclick="toggleDetails()" title="显示/隐藏金额明细">
        ...
        <h2>投资组合调仓展示 (当前总市值: <span id="totalValueText">{total_val:.1f}万</span>)</h2>
    ...
    <script>
        ...
        let showDetails = localStorage.getItem('showLabelDetails') !== null ? localStorage.getItem('showLabelDetails') === 'true' : {'true' if show_details else 'false'};
        ...
        function updateToggleIcon() {{
            ...
            if (showDetails) {{
                ...
                btn.title = "隐藏金额明细";
                if (totalValText) totalValText.innerText = "{total_val:.1f}万";
            }} else {{
                icon.innerHTML = '<path d="M12 7c2.76 0 5 2.24 5 5 0 .65-.13 1.26-.36 1.82l2.92 2.92C21.27 15.11 22.7 13.68 24 12c-1.73-4.39-6-7.5-11-7.5-1.4 0-2.74.25-3.98.7l2.16 2.16C11.18 7.13 11.59 7 12 7zM2 4.27l2.28 2.28.46.46C3.08 8.3 1.78 10.02 1 12c1.73 4.39 6 7.5 11 7.5 1.55 0 3.03-.3 4.38-.84l.42.42L19.73 22 21 20.73 3.27 3 2 4.27zM7.53 9.8l1.55 1.55c-.05.21-.08.43-.08.65 0 1.66 1.34 3 3 3 .22 0 .44-.03.65-.08l1.55 1.55c-.67.33-1.41.53-2.2.53-2.76 0-5-2.24-5-5 0-.79.2-1.53.53-2.2zm4.34-1.2l3.53 3.53c-.07-.13-.13-.26-.13-.41 0-1.66-1.34-3-3-3-.15 0-.28.06-.41.13z"/>';
                btn.title = "显示金额明细";
                if (totalValText) totalValText.innerText = "****万";
            }}
        }}
        updateToggleIcon();
        ...
        function toggleDetails() {{
            ...
            trendChart.setOption({{
                series: [{{
                    label: {{
                        formatter: function(params) {{
                            return showDetails ? params.value : '***';
                        }}
                    }}
                }}]
            }});
            ...
            if (currentSelectedDate) {{
                showRebalanceDetail(currentSelectedDate);
            }}
        }}
        ...
        trendChart.setOption({{
            ...
            tooltip: {{ 
                ...
                formatter: function(params) {{
                    ...
                    let displayVal = showDetails ? val + "万" : "***万";
                    return date + "<br/>总市值: " + displayVal;
                }}
            }},
        ...
        function showRebalanceDetail(date) {{
            ...
            document.getElementById('statVol').innerText = showDetails ? info.trade_vol : "****";
            document.getElementById('statTurn').innerText = info.turnover;
            
            const effSpan = document.getElementById('statEff');
            effSpan.innerText = showDetails ? ((info.efficiency > 0 ? "+" : "") + info.efficiency) : "****";
        ...
        function renderTable(prefix, data) {{
            ...
            data.forEach(row => {{
                ...
                displayCols.forEach(c => {{
                    ...
                    if (!showDetails && ['持有数量', '当前(万)', '现金(万)', '收益(万)'].includes(c)) {{
                        td.innerText = "****";
                    }} else {{
                        td.innerText = val;
                    }}
            ...
            if (sumRow) {{
                ...
                sumMetrics.forEach(m => {{
                    ...
                    if (!showDetails && ['现金(万)', '收益(万)'].includes(m.key)) {{
                        tdVal.innerText = "**** " + (m.unit ? ' ' + m.unit : '');
                    }} else {{
